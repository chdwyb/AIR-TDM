...
class EncoderTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # plt.figure(1)
        # C = x1.shape[1]
        # q_ = x.transpose(-1, -2).reshape(B, C, H1, W1).cpu().detach()
        # for i in range(30):
        #     plt.imshow(q_[0][i])
        #     print(q_[0][i].shape)
        #     plt.axis('off')
        #     plt.savefig(f'E://shangjiao/attention/{i + 1}.png')
        #     plt.close()
...
if __name__ == '__main__':
    ...
    image = Image.open('E://shangjiao/x_15-correct-correct.png').convert('RGB')
    image = ttf.to_tensor(image).unsqueeze(0).cuda()
    print(image.shape)
